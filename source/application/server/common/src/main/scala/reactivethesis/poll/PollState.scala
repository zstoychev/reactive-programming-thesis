...
object PollState {
  ...
  def updateState(state: Option[PollState], event: PollEvent) = (state, event) match {
    case (None, PollStarted(id, description, options)) => Some(PollState(Poll(id, description, options, List.empty)))
    ...
    case (Some(ps@PollState(poll, _)), PollAnswerUpdated(id, name, optionsAnswers)) =>
      poll.answers.find(_.id == id) match {
        case Some(answer) =>
          val answers = PollAnswer(id, name, optionsAnswers) :: poll.answers.filter(_.id != id)
